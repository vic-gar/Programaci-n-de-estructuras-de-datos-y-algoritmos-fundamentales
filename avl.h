...
Node<T>* Node<T>::check_tree(T *check_val, Node<T> *parent, bool *checked) {
	Node<T>* a = 0;
    if (left != 0) {
		left->check_tree(check_val,this,checked);
	}
    if (right != 0) {
		right->check_tree(check_val,this,checked);
	}
...
void AVL<T>::add(T val) {
	if (root != 0) {
		if (!root->find(val)) {
			Node <T> *temp;
			T check_val = root->value;
			bool aux = false;
			root->add(val);
			root->max_depth();
			temp = root->check_tree(&check_val, 0, &aux);
			if (aux && temp && check_val == root->value){
				root = temp;
			}
			root->max_depth();
		}
...
void AVL<T>::remove(T val) {
	if (root != 0) {
		...
		root->max_depth();
		Node <T> *temp;
		T check_val = root ? root->value : val;
		bool aux = false;
		temp = root ? root->check_tree(&check_val, 0, &aux) : nullptr;
		if (aux && root && check_val == root->value){
			root = temp;
		}
		if (root){
			root->max_depth();
